...
def load_images_croppedyale():
    ...
    try:
        os.chdir('CroppedYale')  #Go into the CroppedYale directory
        subdirs = sorted(os.listdir())  #List the directories in the above directory
        pics = OrderedDict()  #To make ordered key, value pairs of persons in sequential order
        u_fname_suffixes, i_fname_suffixes = OrderedSet(), OrderedSet()
        image_resolution = None
        first_iter = True
        for subdir in subdirs: # Iterate over all folders (persons)
            pics[subdir] = OrderedDict()  #persons are stored in ordered dictionary
            os.chdir(subdir)  #Get inside the persons folder (images of person)
            file_names = [x for x in os.listdir() if x.endswith('.pgm') and 'Ambient' not in x] # get all pictures except the ambient
            current_suffixes = sorted(x[len(subdir) + 1 : -4] for x in file_names)##Does it get B01, B02 ...??
            for fname_suffix in current_suffixes: # Iterate over files (images)
                fname = subdir + '_' + fname_suffix + '.pgm'
                pic = imageio.imread(fname)  ##Read the image
